...
export const EmployeeLoginView: React.FC = () => {
  const { currentRole, setCurrentRole, setAuthStep, triggerToast } = useApp();
  ...
  const handleRoleChange = (r: UserRole) => {
    setCurrentRole(r);
    triggerToast(`Switched login portal to: ${r === 'team_leader' ? 'Team Leader / Supervisor' : 'Telecaller / SDR'}`);
  };

  // Instant Google Login -> Step 2
  const handleGoogleLogin = () => {
    setIsLoading(true);
    triggerToast(`✓ Authenticating ${isLeader ? 'Ramesh Sharma (Team Leader)' : 'Arjun Kumar (Telecaller)'}...`);
    setTimeout(() => {
      setIsLoading(false);
  ...
  const handleSendOtp = (e: React.FormEvent) => {
    e.preventDefault();
    setIsOtpSent(true);
    triggerToast(`✓ 6-digit OTP sent to ${mobileNumber || (isLeader ? '+91 98450 00112' : '+91 98765 43210')}`);
  };
  ...
  const handlePasswordSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    triggerToast(`✓ Verified ${isLeader ? 'Team Leader' : 'Employee'} credentials! Proceeding to Face Recognition...`);
    setTimeout(() => {
      setIsLoading(false);
      setAuthStep('FACE_SCAN');
    }, 300);
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col justify-between max-w-md mx-auto relative overflow-hidden font-sans text-slate-800 selection:bg-[#5B3DF5]/20 pb-6">
      ...
      {/* 1. Gradient Top Header */}
      <div className={`text-white pt-9 pb-14 px-6 rounded-b-[36px] shadow-xl relative overflow-hidden transition-all duration-500 ${
        isLeader 
          ? 'bg-gradient-to-b from-[#0A2540] via-[#0F3258] to-[#0A2540]' 
          : 'bg-gradient-to-b from-[#381D88] via-[#4F2BB9] to-[#3B1F8F]'
      }`}>
        ...
        <div className="flex items-center justify-between relative z-10">
          <div className="flex items-center gap-3">
            ...
            <div className="w-12 h-12 rounded-2xl bg-white text-[#0A2540] p-2 flex items-center justify-center shadow-lg shadow-black/20 flex-shrink-0">
              <ScanFace className={`w-7 h-7 stroke-[2] ${isLeader ? 'text-[#00A88B]' : 'text-[#4F2BB9]'}`} />
            </div>
            <div>
              <h1 className="font-display font-black text-xl text-white tracking-tight leading-tight">
                {isLeader ? 'TeamLeader Portal' : 'SmartAttend'}
              </h1>
              <p className="text-[11px] text-white/80 font-medium tracking-wide">
                {isLeader ? 'Supervisor Face Biometric' : 'Face Recognition Attendance'}
              </p>
            </div>
          </div>

          {/* Role Status Tag */}
          <div className={`px-2.5 py-1 rounded-full text-[10px] font-mono font-bold flex items-center gap-1 border ${
            isLeader ? 'bg-[#00C9A7]/20 text-[#00C9A7] border-[#00C9A7]/40' : 'bg-white/10 text-white border-white/20'
          }`}>
            <Shield className="w-3 h-3" />
            <span>{isLeader ? 'SUPERVISOR' : 'TELECALLER'}</span>
          </div>
        </div>
      </div>

      {/* 2. Employee / Leader Login Card Body */}
      <div className="px-5 -mt-8 relative z-20 flex-1 flex flex-col justify-center">
        <div className="nexus-card p-6 bg-white border border-slate-200/80 shadow-2xl rounded-3xl space-y-4">
          ...
          {/* Portal Selector Toggle on Top of Card */}
          <div className="grid grid-cols-2 p-1 bg-slate-100 rounded-2xl border border-slate-200">
            <button
              type="button"
              onClick={() => handleRoleChange('telecaller')}
              className={`py-2 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-1.5 ${
                !isLeader 
                  ? 'bg-white text-[#4F2BB9] shadow-xs' 
          ...
          <div className="text-center space-y-0.5 pt-1">
            <h2 className="font-display font-black text-2xl text-[#0A2540] tracking-tight">
              {isLeader ? 'Supervisor Login' : 'Employee Login'}
            </h2>
            <p className="text-xs text-slate-500 font-medium">
              {isLeader ? 'Sign in to access team roster & approvals' : 'Login to your account to continue'}
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <button
              ...
              className={`p-3 rounded-2xl border text-center flex flex-col items-center justify-center gap-1.5 transition-all ${
                activeLoginType === 'gmail'
                  ? isLeader ? 'border-[#00C9A7] bg-[#E6FAF6] shadow-xs' : 'border-[#4F2BB9] bg-[#EDE9FE] shadow-xs'
            ...
            >
              ...
              <span className={`text-[10px] font-bold leading-tight ${
                activeLoginType === 'gmail' ? (isLeader ? 'text-[#00A88B]' : 'text-[#4F2BB9]') : 'text-slate-700'
              }`}>
                Login with Gmail
              </span>
            </button>
            ...
            >
              <Smartphone className={`w-5 h-5 ${
                activeLoginType === 'mobile' ? (isLeader ? 'text-[#00A88B]' : 'text-[#4F2BB9]') : 'text-slate-500'
              }`} />
              <span className={`text-[10px] font-bold leading-tight ${
                activeLoginType === 'mobile' ? (isLeader ? 'text-[#00A88B]' : 'text-[#4F2BB9]') : 'text-slate-700'
              }`}>
                Mobile Number
              </span>
            </button>
            ...
            >
              <Mail className={`w-5 h-5 ${
                activeLoginType === 'password' ? (isLeader ? 'text-[#00A88B]' : 'text-[#4F2BB9]') : 'text-slate-500'
              }`} />
              <span className={`text-[10px] font-bold leading-tight ${
                activeLoginType === 'password' ? (isLeader ? 'text-[#00A88B]' : 'text-[#4F2BB9]') : 'text-slate-700'
              }`}>
                Password Login
              </span>
            </button>
          ...
          >
            ...
            <span>{isLoading ? 'Signing In...' : `Continue with Google (${isLeader ? 'Supervisor' : 'Telecaller'})`}</span>
          </button>
            ...
            <form onSubmit={isOtpSent ? handleVerifyOtp : handleSendOtp} className="space-y-3">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={isLeader ? "Enter supervisor mobile number" : "Enter telecaller mobile number"}
              ...
              <button
                type="submit"
                disabled={isLoading}
                className={`w-full py-3.5 rounded-2xl text-white font-extrabold text-xs shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-all ${
                  isLeader ? 'bg-[#0A2540] hover:bg-[#0F3258]' : 'bg-[#4F2BB9] hover:bg-[#3D1E99]'
                }`}
              >
            ...
            <form onSubmit={handlePasswordSubmit} className="space-y-3">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={isLeader ? "Enter supervisor email (e.g. ramesh.sharma@...)" : "Enter telecaller email"}
                  className="w-full bg-slate-50 border border-slate-200 rounded-2xl pl-10 pr-4 py-3 text-xs text-slate-800 focus:outline-none focus:border-[#00C9A7] font-medium"
                />
              </div>
              ...
              <button
                ...
                className={`w-full py-3.5 rounded-2xl text-white font-extrabold text-xs shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-all ${
                  isLeader 
                    ? 'bg-[#0A2540] hover:bg-[#0F3258] text-[#00C9A7] border border-[#00C9A7]/40' 
                    : 'bg-[#4F2BB9] hover:bg-[#3D1E99]'
                }`}
              >
                <span>Login as {isLeader ? 'Team Leader' : 'Telecaller'}</span>
                <ArrowRight className="w-4 h-4" />
              </button>
            </form>
          )}
      ...
      <div className="px-5 mt-4">
        <div className="bg-white border border-slate-200 rounded-3xl p-4 shadow-sm">
          ...
          <div className="flex items-center justify-between text-center relative">
            ...
            >
              <div className="w-10 h-10 rounded-2xl bg-slate-100 border border-slate-200 flex items-center justify-center text-[#0A2540] relative mb-1.5 shadow-2xs">
                <UserCheck className="w-5 h-5" />
                <span className="w-4 h-4 rounded-full bg-[#0A2540] text-white text-[9px] font-black absolute -top-1 -right-1 flex items-center justify-center">1</span>
              </div>
            ...
            >
              <div className="w-10 h-10 rounded-2xl bg-[#E6FAF6] border border-[#00C9A7]/40 flex items-center justify-center text-[#00A88B] relative mb-1.5 shadow-2xs">
                ...
                <span className="w-4 h-4 rounded-full bg-[#00C9A7] text-[#0A2540] font-black text-[9px] flex items-center justify-center">2</span>
              </div>
